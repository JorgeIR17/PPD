...
void actualizar_feromonas(double **feromonas, int **rutas, double longitudes[NUM_HORMIGAS]) 
{
    for (int i = 0; i < NUM_PUNTOS; i++) 
    {
        for (int j = 0; j < NUM_PUNTOS; j++) 
...
int main() 
{
    ...
    for (int iter = 0; iter < MAX_ITERACIONES; iter++) 
    {
        ...
        actualizar_feromonas(feromonas, rutas, longitudes);
